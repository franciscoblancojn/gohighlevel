...
export class Gohighlevel {
    ...
    onBusinessUpdate = async ({
        id,
        data,
    }: IGohighlevelFunctions["onBusinessUpdate"]["props"]): IGohighlevelFunctions["onBusinessUpdate"]["respond"] => {
        const result = await this.onRequest({
            url: `/businesses/${id}`,
            method: "PUT",
            body: {
            ...
            ifError: (result) => {
                if (!result?.business?.id) {
                    throw new Error(
                        "Error updating business: " + JSON.stringify(result),
                    );
                }
            },
        });
